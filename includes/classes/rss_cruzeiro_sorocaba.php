<?php 
	...
	class RSS_JCruzSor extends Password {
		...
		public function saveXML() {
			try {
					$feed = "http://www.jornalcruzeiro.com.br/rss/27";
					$xml = simplexml_load_file($feed);
					$lastBuildDate = date_format(date_create(strval($xml->channel->pubDate)),"Y-m-d");
					$result = true;
					$rss = new RSS_JCruzSor($db);
					
					$sql = "SELECT id from rss_cruzeiro_sorocaba WHERE data_item = :data_item";			    
					...
					*/
					...
					if ($checkBuildDate == 0) {
						foreach($xml->xpath("//item") as $item) {
							$sql = "INSERT INTO rss_cruzeiro_sorocaba (title, link, data_item, arquivo_imagem) VALUES (:title, :link, :data_item, :arquivo_imagem)";
							$stmt = $this->_db->prepare($sql);
	    ...
	    public function listar() {
	    	try {
				...
				$sql .= "FROM rss_cruzeiro_sorocaba ORDER BY id DESC";
				$stmt = $this->_db->prepare($sql);
				$stmt->execute();
	    ...
	    public function carregar($id) {
		    try {
		        	$sql  = "SELECT id, title, link, destaque, data_item, exibir ";
					$sql .= "FROM rss_cruzeiro_sorocaba WHERE id = :id ";
			    	$stmt = $this->_db->prepare($sql);
	    ...
	    public function save(RSS_JCruzSor $c) {
	    	try {
	    	
	    		$sql = "INSERT INTO rss_cruzeiro_sorocaba (title, link, dataItem) VALUES (:title, :link, :dataItem)";
	    		$stmt = $this->_db->prepare($sql);	    	
	    ...
	    public function update(RSS_JCruzSor $c) {
	    	try {
	    		$sql  = "UPDATE rss_cruzeiro_sorocaba SET destaque = :destaque, ";
		...
		public function ocultar($id) {
	    	try {
	    		$sql  = "UPDATE rss_cruzeiro_sorocaba SET exibir = 0 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
		...
		public function exibir($id) {
	    	try {
	    		$sql  = "UPDATE rss_cruzeiro_sorocaba SET exibir = 1 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
	    ...
	    public function clearAll() {
	    	try {
	    			$sql  = "UPDATE rss_cruzeiro_sorocaba SET exibir = 0";
	    			$stmt = $this->_db->prepare($sql);
	    			$result = $stmt->execute();			
	    			return $result;
	    	} catch (PDOException $e) {
