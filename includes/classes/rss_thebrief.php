<?php 
	...
	class RSS_Thebrief extends Password {
		...
		public function saveXML() {
			try {
					$feed = "http://rss.thebrief.com.br/feed";
					$xml = simplexml_load_file($feed);
					$lastBuildDate = date_format(date_create(strval($xml->channel->lastBuildDate)),"Y-m-d");
					$result = true;
					$rss = new RSS_Thebrief($db);
					
					$sql = "SELECT id from rss_thebrief WHERE data_item = :data_item";			    
					...
					*/
					...
					if ($checkBuildDate == 0) {
						foreach($xml->xpath("//item") as $item) {
							$sql = "INSERT INTO rss_thebrief (title, link, data_item, arquivo_imagem) VALUES (:title, :link, :data_item, :arquivo_imagem)";
							$stmt = $this->_db->prepare($sql);
	    ...
	    public function carregar($id) {
		    try {
		        	$sql  = "SELECT id, title, link, destaque, data_item, exibir ";
					$sql .= "FROM rss_thebrief WHERE id = :id ";
			    	$stmt = $this->_db->prepare($sql);
	    ...
	    public function save(rss_thebrief $c) {
	    	try {
	    	
	    		$sql = "INSERT INTO rss_thebrief (title, link, dataItem) VALUES (:title, :link, :dataItem)";
	    		$stmt = $this->_db->prepare($sql);	    	
	    ...
	    public function update(rss_thebrief $c) {
	    	try {
	    		$sql  = "UPDATE rss_thebrief SET destaque = :destaque, ";
		...
		public function ocultar($id) {
	    	try {
	    		$sql  = "UPDATE rss_thebrief SET exibir = 0 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
		...
		public function exibir($id) {
	    	try {
	    		$sql  = "UPDATE rss_thebrief SET exibir = 1 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
	    ...
	    public function clearAll() {
	    	try {
	    			$sql  = "UPDATE rss_thebrief SET exibir = 0";
	    			$stmt = $this->_db->prepare($sql);
	    			$result = $stmt->execute();			
	    			return $result;
	    	} catch (PDOException $e) {
