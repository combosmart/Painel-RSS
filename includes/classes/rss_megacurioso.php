<?php 
	...
	class RSS_Megacurioso extends Password {
		...
		public function saveXML() {
			try {
					$feed = "https://rss.megacurioso.com.br/feed";

					if (is_valid_url($feed)) {

						$xml = simplexml_load_file($feed);
						$lastBuildDate = date_format(date_create(strval($xml->channel->lastBuildDate)),"Y-m-d");
						$result = true;
						$rss = new RSS_Megacurioso($db);
						
						$sql = "SELECT id from rss_megacurioso WHERE data_item = :data_item";			    
						...
						*/
						...
						if ($checkBuildDate == 0) {
							foreach($xml->xpath("//item") as $item) {
								...
								$sql = "INSERT INTO rss_megacurioso 
	    ...
	    public function listar() {
	    	try {
				...
				$sql .= "FROM rss_megacurioso ORDER BY id DESC";
				$stmt = $this->_db->prepare($sql);
				$stmt->execute();
	    ...
	    public function carregar($id) {
		    try {
		        	$sql  = "SELECT id, title, link, destaque, data_item, exibir ";
					$sql .= "FROM rss_megacurioso WHERE id = :id ";
			    	$stmt = $this->_db->prepare($sql);
	    ...
	    public function update(RSS_Megacurioso $c) {
	    	try {
	    		$sql  = "UPDATE rss_megacurioso SET destaque = :destaque, ";
		...
		public function ocultar($id) {
	    	try {
	    		$sql  = "UPDATE rss_megacurioso SET exibir = 0 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
		...
		public function exibir($id) {
	    	try {
	    		$sql  = "UPDATE rss_megacurioso SET exibir = 1 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
	    ...
	    public function clearAll() {
	    	try {
	    			$sql  = "UPDATE rss_megacurioso SET exibir = 0";
	    			$stmt = $this->_db->prepare($sql);
	    			$result = $stmt->execute();			
	    			return $result;
	    	} catch (PDOException $e) {
