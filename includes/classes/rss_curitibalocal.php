<?php 
	class RSS_CuritibaLocal extends Password {
		...
		public function saveXML() {
			try {
					$feed = "http://curitibacult.com.br/tag/curitiba/feed/";
					
					if (test_url($feed)) {
						$xml = simplexml_load_file($feed);
						$lastBuildDate = date_format(date_create(strval($xml->channel->lastBuildDate)),"Y-m-d");
						$result = true;
						$rss = new RSS_CuritibaLocal($db);
						
						$sql = "SELECT id from rss_curitibalocal WHERE data_item = :data_item";			    
	    ...
	    public function listar() {
	    	try {
				...
				$sql .= "FROM rss_curitibalocal ORDER BY id desc";
				$stmt = $this->_db->prepare($sql);
				$stmt->execute();
	    ...
	    public function carregar($id) {
		    try {
		        	$sql  = "SELECT id, title, link, destaque, data_item, exibir ";
					$sql .= "FROM rss_curitibalocal WHERE id = :id ";
			    	$stmt = $this->_db->prepare($sql);
	    ...
	    public function save(rss_curitibalocal $c) {
	    	try {
	    	
	    		$sql = "INSERT INTO rss_curitibalocal (title, link, dataItem) VALUES (:title, :link, :dataItem)";
	    		$stmt = $this->_db->prepare($sql);	    	
	    ...
	    public function update(rss_curitibalocal $c) {
	    	try {
	    		$sql  = "UPDATE rss_curitibalocal SET destaque = :destaque, ";
		...
		public function ocultar($id) {
	    	try {
	    		$sql  = "UPDATE rss_curitibalocal SET exibir = 0 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
		...
		public function exibir($id) {
	    	try {
	    		$sql  = "UPDATE rss_curitibalocal SET exibir = 1 WHERE id = :id	";
	    		$stmt = $this->_db->prepare($sql);
	    		$result = $stmt->execute(array(						
							':id' => $id
						));			
	    ...
	    public function clearAll() {
	    	try {
	    			$sql  = "UPDATE rss_curitibalocal SET exibir = 0";
	    			$stmt = $this->_db->prepare($sql);
	    			$result = $stmt->execute();			
	    			return $result;
	    	} catch (PDOException $e) {
