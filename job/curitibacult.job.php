<?php
/**
 * Job de obtenção de feed do Curitiba Cult
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_curitibacult WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Curitiba Cult (Geral)";
		$body    = "<p>O job de obtenção de feeds do parceiro Curitiba Cult (Geral) foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
