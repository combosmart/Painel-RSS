<?php
/**
 * Job de obtenção de feed do Curitiba Cult (Local)
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_curitibalocal WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Curitiba Cult (Local)";
		$body    = "<p>O job de obtenção de feeds do parceiro Curitiba Cult (Local) foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
