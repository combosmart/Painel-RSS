<?php
/**
 * Job de obtenção de feed do Minha Série
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_minhaserie WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Minha Série";
		$body    = "<p>O job de obtenção de feeds do parceiro Minha Série foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
