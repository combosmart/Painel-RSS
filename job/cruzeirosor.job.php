<?php
/**
 * Job de obtenção de feed do Megacurioso
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_cruzeiro_sorocaba WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Cruzeiro do Sul Sorocaba";
		$body    = "<p>O job de obtenção de feeds do parceiro Cruzeiro do Sul Sorocaba foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
