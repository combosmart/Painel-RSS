<?php
/**
 * Job de obtenção de feed do The Brief
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_thebrief WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed The Brief";
		$body    = "<p>O job de obtenção de feeds do parceiro The Brief foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
