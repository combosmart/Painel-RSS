<?php
/**
 * Job de obtenção de feed do Cruzeiro do Sul Geral
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_cruzeiro_geral WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Cruzeiro do Sul Geral";
		$body    = "<p>O job de obtenção de feeds do parceiro Cruzeiro do Sul Geral foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
