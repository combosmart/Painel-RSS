<?php
/**
 * Job de obtenção de feed do Tecmundo
 *
...
if ($isValidURL) {
	...
	foreach($xml->xpath("//item") as $item) {
		...
		$sql = "SELECT id from rss_tecmundo WHERE uid = :uid";			    
		$stmt = $db->prepare($sql);
		$stmt->execute(array(						
			':uid' => $uid
		));			
	...
	if ($result) {
		...
		$subject = "Combo Vídeos - Execução de Job - Feed Tecmundo";
		$body    = "<p>O job de obtenção de feeds do parceiro Tecmundo foi executado com sucesso em " . date("d/m/y G:i:s") . "</p>";				
		$mail = new Mail();
