<?php 	
	...
	$mode = $_GET["mode"];
	$title = 'Combo Vídeos - Administração de Feeds: Curitiba Cult';
	include 'header.php'; 
	...
	if(!empty($_POST['clear'])) {
		$result = $curitibacult->clearAll();
		if ($result) {
			$message = 'Playlist esvaziada com sucesso.';
			$_SESSION['success'] = $message;
			header('Location: rss_curitibacult.php?mode=list');
		} else {
	...
	if(!empty($_POST['edit'])) {
		...
		$curitibacult->setId($_POST['id']);
		$curitibacult->setDestaque($_POST['destaque']);
		$curitibacult->setExibir($_POST['exibir']);
		$curitibacult->setTitle($_POST['title']);
		...
		if (is_uploaded_file($_FILES["post_img"]["tmp_name"])) {
			...
			$curitibacult->setLink($_POST['link']);			
		}
		
		if(!isset($error)){
			$result = $curitibacult->update($curitibacult);
			if ($result) {
			...
			<?php include 'sidebar.php'; ?>
				...
				<section role="main" class="content-body">
					...
					<header class="page-header page-header-left-breadcrumb">
						<div class="right-wrapper">
							<ol class="breadcrumbs">
								...
								<li><span>Administração de Feeds: Curitiba Cult</span></li>								
							</ol>
							...
							<table class="table table-bordered table-striped mb-none" id="datatable-default" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<a href="./rss_curitibacult.php?mode=edit&id=<?php echo $row['id']; ?>"><i class="fa fa-edit" aria-hidden="true" data-toggle="tooltip" title="Editar"></i></a>
											&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
											<?php if ($row['exibir'] == 1) { ?>
												<a href="./rss_curitibacult.php?mode=ocultar&id=<?php echo $row['id']; ?>" ><i class="fa fa-power-off" aria-hidden="true" data-toggle="tooltip" title="Ocultar"></i></a>
											<?php } else { ?>													
												<a href="./rss_curitibacult.php?mode=exibir&id=<?php echo $row['id']; ?>" ><i class="fa fa-power-off" aria-hidden="true" data-toggle="tooltip" title="Exibir"></i></a>
											<?php } ?>											
										</td>
									</tr>
									<?php } ?>
					...
					<?php 
						if ($mode == EDITAR) { 
						if (!(in_array($_SESSION['role_id'], array(ADMINISTRADOR, USUARIO)))) { header('Location: main.php'); }
          				$record = $curitibacult->carregar($_GET["id"]);        
					?>
					<?php if(isset($error)) { 
					...
					<header class="page-header page-header-left-breadcrumb">
						<div class="right-wrapper">
							<ol class="breadcrumbs">
								...
								<li><span>Início</span></li>								
								<li><span><a href="rss_curitibacult.php?mode=list">Administração de Feeds: Curitiba Cult</a></span></li>								
							...
							<footer class="panel-footer">
								<div class="row">
									<div class="col-sm-9 col-sm-offset-3">
										...
										<button type="reset" class="btn btn-default" onclick="window.location='rss_curitibacult.php?mode=list'">Reset</button>
									</div>
								</div>
							</footer>
						</form>
					...
					?>
					<?php if ($mode == OCULTAR) {    
							$result = $curitibacult->ocultar($_GET['id']);
							if ($result) {
								header('Location: rss_curitibacult.php?mode=list');
							}			              
          				}
      				?>
      				<?php if ($mode == EXIBIR) {    
							$result = $curitibacult->exibir($_GET['id']);
							if ($result) {
								header('Location: rss_curitibacult.php?mode=list');
							}			              
          				}
