<?php 	
	...
	$mode = $_GET["mode"];
	$title = 'Combo Vídeos - Administração de Feeds: Curitiba Cult (Local)';
	include 'header.php'; 
	...
	if(!empty($_POST['clear'])) {
		$result = $curitibalocal->clearAll();
		if ($result) {
			$message = 'Playlist esvaziada com sucesso.';
			$_SESSION['success'] = $message;
			header('Location: rss_curitibalocal.php?mode=list');
		} else {
	...
	if(!empty($_POST['edit'])) {
		...
		$curitibalocal->setId($_POST['id']);
		$curitibalocal->setDestaque($_POST['destaque']);
		$curitibalocal->setExibir($_POST['exibir']);
		$curitibalocal->setTitle($_POST['title']);
		...
		if (is_uploaded_file($_FILES["post_img"]["tmp_name"])) {
			...
			$curitibalocal->setLink($_POST['link']);			
		}
		
		if(!isset($error)){
			$result = $curitibalocal->update($curitibalocal);
			if ($result) {
				...
				<section role="main" class="content-body">
					...
					<header class="page-header page-header-left-breadcrumb">
						<div class="right-wrapper">
							<ol class="breadcrumbs">
								...
								<li><span>Administração de Feeds: Curitiba Cult (Local)</span></li>								
							</ol>
					...
					<section class="panel">
						<header class="panel-heading">
							<h2 class="panel-title">Feed de publicações do Curitiba Cult <a class="mb-1 mt-1 mr-1 btn btn-default" data-toggle="modal" data-target="#modalBootstrap" href="#">Limpar Feed</a></h2>
						</header>
						<!-- MODAL LIMPAR FEED -->
						<form method="post">
							...
							<table class="table table-bordered table-striped mb-none" id="datatable-default" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<a href="./rss_curitibalocal.php?mode=edit&id=<?php echo $row['id']; ?>"><i class="fa fa-edit" aria-hidden="true" data-toggle="tooltip" title="Editar"></i></a>
											&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
											<?php if ($row['exibir'] == 1) { ?>
												<a href="./rss_curitibalocal.php?mode=ocultar&id=<?php echo $row['id']; ?>" ><i class="fa fa-power-off" aria-hidden="true" data-toggle="tooltip" title="Ocultar"></i></a>
											<?php } else { ?>													
												<a href="./rss_curitibalocal.php?mode=exibir&id=<?php echo $row['id']; ?>" ><i class="fa fa-power-off" aria-hidden="true" data-toggle="tooltip" title="Exibir"></i></a>
											<?php } ?>											
										</td>
									</tr>
									<?php } ?>
					...
					<?php 
						if ($mode == EDITAR) { 
						if (!(in_array($_SESSION['role_id'], array(ADMINISTRADOR, USUARIO)))) { header('Location: main.php'); }
          				$record = $curitibalocal->carregar($_GET["id"]);        
					?>
					<?php if(isset($error)) { 
					...
					<header class="page-header page-header-left-breadcrumb">
						<div class="right-wrapper">
							<ol class="breadcrumbs">
								...
								<li><span>Início</span></li>								
								<li><span><a href="rss_curitibalocal.php?mode=list">Administração de Feeds: Curitiba Cult (Local)</a></span></li>								
							...
							<footer class="panel-footer">
								<div class="row">
									<div class="col-sm-9 col-sm-offset-3">
										...
										<button type="reset" class="btn btn-default" onclick="window.location='rss_curitibalocal.php?mode=list'">Reset</button>
									</div>
								</div>
							</footer>
						</form>
					...
					?>
					<?php if ($mode == OCULTAR) {    
							$result = $curitibalocal->ocultar($_GET['id']);
							if ($result) {
								header('Location: rss_curitibalocal.php?mode=list');
							}			              
          				}
      				?>
      				<?php if ($mode == EXIBIR) {    
							$result = $curitibalocal->exibir($_GET['id']);
							if ($result) {
								header('Location: rss_curitibalocal.php?mode=list');
							}			              
          				}
